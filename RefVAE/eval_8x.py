...
parser.add_argument('--patch_size', type=int, default=64, help='patch size for memory safety') # change
parser.add_argument('--stride', type=int, default=4, help='overlap to prevent seam lines') # change
    ...
    for i in range(len(lr_image)):
        ...
        if len(ref_image) != 0 and opt.use_ref:
            ...
            for j in range(len(ref_image)):
                ...
                # --- CHANGE ---
                base_lr = os.path.basename(lr_image[i]).split('.')[0]
                ref_name = os.path.basename(ref_image[j]).split('.')[0]
                lr_name = f"{base_lr}_using_{ref_name}"
                # -------------------------
                ...
                lr_name = os.path.basename(lr_image[i]).split('.')[0]
    ...
    for iteration, batch in enumerate(test_dataloader, 1):
        ...
        if opt.chop_forward:
            ...
            with torch.no_grad():
                ...
                # Stitch the tiles back together into a massive final image
                out_box = torch.cat(out_box, 0)
                SR = recompose_tensor(out_box, opt.up_factor * img_height, opt.up_factor * img_width, opt.stride*opt.up_factor)
        else:
            # The old, memory-crashing method
